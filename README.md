...
**Agentic loop 的健壮性设计**（参考成熟 Agent 框架）：① 网络瞬时失败（限流/5xx/超时）会自动按退避策略重试，不会动不动报错；② 模型如果连续用相同参数重复调用同一个工具，会被注入"换种做法"的提醒打断死循环；③ 每个工具结果带 `[工具名 成功/失败]` 标记、超长结果自动裁剪，避免上下文膨胀；④ 单轮工具调用数量有上限；⑤ 对话历史过长时用一次 LLM 调用把旧内容压缩成记忆摘要，比直接丢弃更能记住你交代过的事；⑥ 指令含糊或要做有破坏性/不可逆的操作时，会先**向你提问确认**（用 `/opencraft answer <回答>` 回复，90 秒不回答它会按合理假设继续）；⑦ 多步任务它会用 `task_plan` 列计划并实时更新进度，不会做乱做重复。
...
| `/opencraft answer <回答>` | 回答助手向你提出的确认问题（它问完会暂停等你） |
