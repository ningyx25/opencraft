...
详细日志包含固定种子、自然出生点、地形勘察、配置方块位置、出生点掷点、助手/主人落点、工具序列、周期状态、最终背包与验证结果。
- LLM 逐轮请求/回复/重试/失败：`run/logs/e2e-debug-<task>.log`（e2e 服务器默认开启调试日志，任务结束自动另存；"助手未产出回复"类失败必看此文件区分模型/端点问题）。

> 说明：固定种子只保证世界地形固定；vanilla 新玩家落点用实体随机源螺旋搜索，同一存档每次落点不同。harness 会**反复掷点直到落点 20 格半径内满足勘察条件**（日志中的"出生点掷点 #N"），避免落到无树山顶这类"环境不可能完成"的假失败。
