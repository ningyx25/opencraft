...
| `Prompts.java` | system 提示词组装（对应 dsh `system-prompt/`：有序小节拼装）——**KV 前缀缓存友好拆分**：`staticSystem` 只拼跨轮恒定不变的静态段（`# Identity` → 预设 persona → `# Capabilities`（插件片段）→ `# Skills`（技能库）），整个 loop 生命周期组装一次、绝不就地重建；`formatGameContext` 产出动态 `# Game Context` 段（`## Player State` + `## Assistant State` + 插件状态）,由 `AgentRuntime.startLoop` 随首轮提问注入消息流（之后作为不可变历史被缓存）。插件提示词片段仍在各插件（自带 `##`），预设 persona 仍在 `presets/` 预设类内（自带 `#`）。 |
| `GameContext.java` | 模型可见的**动态游戏上下文**（对应 dsh `context/`）：观察世界/背包 → json 数据段（`playerState` 主人段：位置/环境/时间/生命饥饿/经验/模式/身体/效果/装备/注视/背包摘要；`assistantState` 助手段：坐标/朝向/移动 + 环境 + 近旁方块（带相对坐标）+ 大范围方块计数 + 附近实体 + 按槽位背包装备清单，完整吸收原 player_look；背包是摘要，精确完整视图由插件工具 `player_inventory` 按需提供）。完整段只在**任务首轮**随提问注入消息流（见 `AgentRuntime.startLoop`）；`stateDigest` 产出单行轻量摘要，由 runtime 在后续轮次**仅在变化时**追加为 `[Current State]` 消息（system 恒定、历史 append-only → KV 前缀缓存延伸到全部历史）。状态 JSON 同步落快照 `logs/opencraft/player.json` 与 `assistant.json`（`logging/StateSnapshots`）。从原 `Prompts` 拆出——观察（context）与拼装（system-prompt）分离。 |
...
| `LoopSession.java` | 单次提问（一次 loop 任务）的共享状态（原 `AgentRuntime` 内部类 `LoopContext` 提升为顶层类）：消息列表、忙锁键、静态 system（跨轮恒定）、上一条尾部状态观测摘要（`lastStateDigest`）、计划（plan）、终止守卫计数、暂停标记（ask/action），以及本次任务装配的 `hooks` 列表；并提供 `asyncActionInFlight()` 判定（终止守卫与延迟动作共用）。 |
...
| `TaskPlan.java` | 任务计划数据模型（`task_plan` 核心工具）：模型以 `{steps:[{content,status}]}` 整单替换维护结构化步骤清单（pending/in_progress/completed），负责参数校验解析、摘要与进行中步骤（随工具结果回显+尾部观测，不进 system）。由 `hooks/TaskPlanHook` 贡献工具/认领调用并写回 `LoopSession.plan`。纯 Java 可单测。 |
