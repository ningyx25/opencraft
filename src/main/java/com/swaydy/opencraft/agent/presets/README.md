...
| `GeneralAgent.java` | `general_agent` | 像普通玩家一样行动的助手（**默认预设**）：**默认自动跟随主人**（玩家下达任务指令后退出跟随专注执行，完成后回到跟随），行动全部用真实玩家方式完成——移动/挖掘/放置/合成/递物 + 传送到主人身边。观察（位置/环境/近旁方块/实体/背包）由 `Assistant State` 上下文每轮自动提供，定向找坐标用 `player_find`。人设强调「读上下文观察→计划→行动→读上下文确认」、失败换做法不重复调用、含糊先问玩家、多步任务用 task_plan。 | `AssistantControlPlugin`（teleport_to_player）+ 玩家动作 6 个能力族插件（`PlayerMovementPlugin` 移动 / `PlayerPerceptionPlugin` 感知 find / `PlayerWorldPlugin` 挖掘放置 / `PlayerCraftingPlugin` 合成 / `PlayerInventoryPlugin` 背包物品 / `PlayerContainerPlugin` 容器，实现共享 `PlayerActionMechanics`，见 plugins/README） | 250 |
...
- 核心工具（`ask_player`、`task_plan`）由 loop 钩子 `AskPlayerHook` / `TaskPlanHook` 贡献（`hooks/LoopHooks.createDefaults()`），与预设无关；见 `../hooks/README.md`。
