...
| **AiConfigData.java** | **配置的网络传输载体**：Gson 序列化为 JSON 在客户端/服务器间传递 | `record` 与 `AiBlockConfig` 可编辑字段一一对应。安全约定：服务端下发时 `apiKey` 恒为空串、仅以 `apiKeySet` 告知"已设置/未设置"；客户端换密钥须 `apiKeyChanged=true`（留空=清除），否则保存时保留原密钥。`toJson`/`fromJson`/`fromConfig`/`applyTo` |
